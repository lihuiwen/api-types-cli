...
class ApiTypesGenerator {
    ...
    async generateTypeScript(name, jsonData) {
    ...
    async saveTypeFile(name, content) {
    ...
    async generateUsageExample(names) {
        const outputDir = path.resolve(this.options.output);
    ...
    async generateFromApis(apis) {
        ...
        for (const chunk of chunks) {
            await Promise.allSettled(chunk.map(async (api) => {
    ...
    showStatistics(successCount, errorCount, errors) {
        console.log('\n' + chalk.cyan('📊 生成统计'));
